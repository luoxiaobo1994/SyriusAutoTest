...
def ssh(ip, cmds=[], username='syrius', password="syrius", port=22, responseResult=None, timeout=None):
    client = paramiko.SSHClient()
    try:
        # 创建ssh客户端
        # 第一次ssh远程连接时，会提示输入yes或no
        client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        # 密码方式远程连接，timeout源码中默认就是15秒
        ...
        if cmds:  # 有时候不执行命令.
            for command in cmds:
                ...
                if responseResult is not None:
                # if responseResult:  # 控制一下,是否打印消息.
                    for m in out:
...
def Linux_command(ip, command, index=0, port=22, username='syrius', password='syrius', name='', need='',
                  isReturn=False, justResult=False):
    '''用于执行linux命令，并返回执行结果'''
    try:
