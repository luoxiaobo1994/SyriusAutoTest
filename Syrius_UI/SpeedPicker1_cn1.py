...
class SpeedPicker:
    ...
    def do_err(self, err=''):
        ...
        log.info(f"本次随机上报的异常是:{err_type}")
        ...
        while count > 0:
            try:
                ...
                log.info(f"确定上报[{err_type2}]异常吗？")  # 询问弹窗.再次确认弹窗询问的异常和选择的是否一致.
        ...
        while (time.time() - start) < timeout:
            ...
            if view_text == tmp_text:
                ...
                if text not in tmp_text and text:
                    log.debug(f"判断1-特征文本：[{text}]已经不在{pagename}页面，判定界面已跳转，程序未卡屏。")
                    return 1  # 特征文本不在界面内了.也可以跳过了.
                elif (new_text, new_text2) and interset((new_text, new_text2), tmp_text):
                    log.debug(f"特征文本：[{new_text, new_text2}],出现在当前界面。判定界面已跳转，程序未卡屏。")
                    return 1
                ...
                log.debug(f"判断2-特征文本：[{text}]已经不在{pagename}页面，判定界面已跳转，程序未卡屏。")
                return 1  # 页面变化了.
        if pagename == "拣货完成":
            tmp_text = self.get_text()
            if '/' in tmp_text[:-1]:
    ...
    def picking1(self, target='', checktarget=False, ismove=False):
        ...
        while True:
            ...
            if '扫货品/输入' in tmp_text:
                ...
                log.debug(f"流程1----还未开始拣货，当前界面文本：{tmp_text}")
            ...
            if self.driver.element_display((By.XPATH, '//android.widget.EditText')):
                log.debug(f"流程2---点击输入按钮后，弹出键盘的界面文本:{new_text}")
                ls.append(2)
            self.driver.tap((By.XPATH, '//*[starts-with(@text,"A0")]'))
            new_text2 = self.get_text()
            if '扫货品/输入' in new_text2:
                log.debug(f"流程3----点击空白区域，收起键盘后的界面文本。{new_text2}")
                ls.append(3)
            if ls == [1, 2, 3]:
                pass
            else:
                log.debug(f"流程顺序出现了一点问题，检查一下。")
            count += 1
    ...
    def bind_container(self):
        ...
        log.info(f"绑定载物箱流程，请给机器人绑定载物箱。载物箱信息:{bind_info}")
        ...
        while True:
            ...
            if '扫码绑定 载物箱' in tmp_text:
                ...
                try:
                    log.debug(f"箱码输入完成，准备点击[完成]。结束绑定流程。当前页面文本:{self.get_text()}")
                    self.click_view_text("完成", wait=2)
    ...
    def other_situation(self):
        ...
        if len_same(self.get_config()['estop_text'], set(view_text)) >= 2:
            ...
            if len_same(self.get_config()['sp_text'], set(tmp_text)):
                ...
                log.info("发生了一些奇怪的异常，可能需要你自己去检查一下了。")
                self.shoot()
                exit(-500)
