...
def check_info(name):
    print(Linux_command(robot[name], 'cat /etc/syrius/ota/version', index=1, name=f'机器人[{name}]的MoveBase-Version:'))
    print(Linux_command(robot[name], 'cat /opt/syrius/ota/checker/application.yml', index=1, need='env: test'))
    print(Linux_command(robot[name], 'cat /sys/robotInfo/RobotSN', index=1, name=f'机器人[{name}]SN:'))
    if Linux_command(robot[name], 'ls -lh /etc/syrius/calibration_result/robot_sensors.yaml', index=1, name='标定文件检查：'):
...
def check_disk(name):
    res = Linux_command(robot[name], 'df | head -2 | grep /')
...
def check_battery(name):
    ...
    res = Linux_command(robot[name], cmd)
    data = res.split()[-1]
